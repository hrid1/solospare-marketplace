...
const BidRequestRow = ({ bid, handleStatusChange }) => {
  const { title, email, deadline, category, status, price, _id } = bid;

  //   const formattedDate = format(new Date(deadline), "MM/dd/yyyy");
  console.log(status);
      ...
      <td className="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
        <div
          className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2 ${
            ...
            status === "Compleated" && "bg-green-100/60 text-green-500"
        ...
        >
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              ...
              status === "Compleated" && "bg-green-500"
      ...
      <td className="px-4 py-4 text-sm whitespace-nowrap">
        <div className="flex items-center gap-x-6">
          ...
          <button
            ...
            disabled={status === "In Progress" || status === "Compleated"}
          ...
          <button
            ...
            disabled={status === "Rejected" || status === "Compleated"}
