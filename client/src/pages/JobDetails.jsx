...
const JobDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const jobId = _id;
    // 0 Check Same User
    if (buyer?.email === email) return toast.error("Same User!!!");
    ...
    if (parseInt(price) > maxPrice)
      return toast.error("Offer less or atleast maxmum price!");
    // 3. date offer check validation
    if (compareAsc(new Date(startDate), new Date(deadline)) === 1)
      return toast.error("Offer a valid Date");
    const bidData = {
      price,
      email,
      comment,
